...
* Use an existing model to **embed sentences** into a "concept" space.
...
::: {#fig-diffusion-attention style="width: 200px; float: left; margin-right: 1em;"}
...
There are a **lot of details** about all the diffusion model tricks they used, including the noise schedule, sample weighting, and classifier-free diffusion guidance. Their *two-tower* diffusion LCM separates the encoding of previous sentences $x_{<n}^0$ from the denoising process from $x_n^t$ with one transformer tower each. These details are a great resource for someone trying to train transformer-diffusion models for time series data.
...
They also compare to standard, word-token-based LLMs on a summarization task, again with conflicting results, depending on the metric. 
