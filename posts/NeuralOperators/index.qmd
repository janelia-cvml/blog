...
Fixing the source term $f(x) = 1$ and the boundary condition $u(x) = 0$ on the boundary $\partial D$, the problem is to find the pressure field $u(x)$ for a given permeability field $a(x)$.
...
In the context of Neural Operators, the goal is to learn the inverse $\mathsf{L}_a^{-1}f$, with the differential operator $\mathsf{L}_a = -\nabla \cdot (a(x) \nabla (\cdot))$, which maps the parameter function $a(x)$ to the solution $u(x)$.
Note that the Darcy flow with a general permeability field $a(x)$ doesn't have an analytically known fundamental solution, and the mapping from $a(x)$ to $u(x)$ is highly non-linear, so that in order to solve the PDE numerical methods are necessary, such as Neural Operators.
...
Each row is mapped to a positional embedding by the projection $P$, and the results are used as a values $v(y)$ in the convolution operation.
In order to evaluate the output function $u$ at a point $z \in D$, the query point $z$ is supplied to the Neural Operator together with the input function $a(x)$ in the form above to obtain an output value $u(y)$.
...
If more query points are used to evaluate the input function, discretization invariance ensures that the output function becomes more accurate without the need to retrain the Neural Operator, converging to the true solution (where the input is the full function $a$) as the number $L$ of query points goes to infinity.
