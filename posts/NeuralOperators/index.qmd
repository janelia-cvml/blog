...
*Neural Operators [[Kovachki, 2021]](#Kovachki2021) and Fourier Neural Operator [[Li, 2020]](#Li2020) papers were presented by Michael Innerberger and Magdalena Schneider in Janelia CVML 2024-11-01 (and discussed in this blog post), Adaptive Fourier Neural Operator [[Guibas, 2022]](#Guibas2022) and FourCastNet [[Pathak, 2022]](#Pathak2022) papers were presented by Magdalena Schneider and Kristin Branson in Janelia CVML 2024-11-08 (see blog post on [FourCastNet](../FourCastNet/index.qmd))*
...
Fortunately, there are multiple ways to speed up the computation of the convolution based on the structure of the kernel, which we will shortly describe in the following (for details see [Kovachki, 2021](#Kovachki2021)).
...
Drawing from ideas of the fast multipole method (FMM) presented in [Greengard and Rokhlin, 1987](#FMM), Multipole Graph Neural Operators (MGNOs) use a hierarchical sequence of graphs to approximate the convolution operation.
...
[Kovachki, 2021](#Kovachki2021) provides various examples of solving PDEs with Neural Operators.
...
Note that, in order to generate training data, in [Kovachki, 2021](#Kovachki2021) the PDEs were solved with classical numerical methods, such as finite differences, finite elements, or spectral methods; see, e.g., [Bartels, 2018](#Bartels2018).
...
While using data from classical methods to train is one common approach, it is also possible to train neural operators directly from data, which was done, for example, in the FourCastNet paper [[Pathak, 2022]](#Pathak2022), and is an active area of research.
...
For solving the inverse problem for a single state, a classical method (middle column) based on finite element methods took 18 hours to compute the solution [[Kovachki, 2021](#Kovachki2021)].
...
After training of the Neural Operator, which took 12 hours (including generation of training data by classical methods), solving the inverse problem for each new single state takes only 2.5 minutes (right column), making this approach much more efficient than classical methods.
...
Examples of computer vision tasks that have been successfully tackled with Neural Operators include image inpainting, super-resolution, image segmentation and classification [[Chi, 2020]](#Chi2020), [[Guibas, 2022]](#Guibas2022), [[Wei, 2023]](#Wei2023), [[Wong, 2023]](#Wong2023).
