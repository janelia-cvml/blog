...
Simply said, neural operators can be seen as a generalization of convolutional neural networks (CNNs) to continuous domains. While CNNs feature convolutions with small kernels on discrete grids, neural operators have a continuous kernel that is convolved with the whole function in each layer.
...
Although neural operators are designed to learn mappings between functions on continuous domains, for numerical implementation, the continuous functions still needs to be discretized, i.e. represented by a set of query points and corresponding function values. While, in principle, the query points can be arbitrary, in practice, they are often chosen to be equidistantly spaced on a grid for computationl efficiency.
Hence, the learned operator is an approximation to to the true operator $\hat{G}$, operating on
$$
\hat{G} \colon \mathbb{R}^{L_d} \to \mathbb{R}^{L_m} \rightarrow \mathcal{U},
$$
where $L_d$ is the number of query points of the input function, $L_m$ is the number of query points of the output function, and $\mathcal{U}$ is the function space of the output function.
...
Note that same other papers have claimed that neural operators are not necessarily discretization invariant (e.g. [Golland, 2023](#Golland2023)), but this is a topic of ongoing research.
...
![Kernel representation of a GNO (adapted from [[Kovachki, 2021]](#Kovachki2021))](kernel-gno.png){#fig-kernel-gno width=400px fig-align="left"}
...
![Multi-level graph used in MGNOs (adapted from [[Kovachki, 2021]](#Kovachki2021))](kernel-mgno.png){#fig-kernel-gno width=400px fig-align="left"}
...
Drawing from ideas of the fast multipole method (FMM) [[Greengard and Rokhlin, 1987]](#FMM), multi-level neural operators use a hierarchical sequence of graphs to approximate the convolution operation.
...
![Kernel representation of a LNO (adapted from [[Kovachki, 2021]](#Kovachki2021))](kernel-lno.png){#fig-kernel-gno width=400px fig-align="left"}
...
![Kernel representation of a FNO (adapted from [[Li, 2020]](#Li2020))](kernel-fno.png){#fig-kernel-gno width=400px fig-align="left"}
...
A full neural operator architecture has several layers and thus, we don't usually have a direct representation of the fundamental solution, either. This simple example of a Poisson equation was trained with a single layer, so the learned kernel is directly interpretable as the fundamental solution and can be compared to the ground truth. Fig. ? shows both the analytic fundamental solution and the learned kernel. Of note, they are not exactly the same, but qualitatively similar.
